...
5. 环路滤波：De-blocking Filter, Reconstruction Filter
   
6. 熵编码： 通用可变长编码（UVLC）和基于文本的自适应二进制算术编码（CABAC）
...
Set for the very last packet of the access unit indicated by the RTP timestamp, in line with the normal use of the M bit in video formats, to allow an efficient playout buffer handling.

若為IS_ACCESS_UNIT，若只要NAL type為0-5就會設定marker為1define IS_ACCESS_UNIT(x) (((x) > 0x00) && ((x) < 0x06))
...
例如 H264的采样率为 90khz, 帧率 frame rate =15
那么每个包的时间戳 Timestamp 的步长约为 90000/15 = 6000
